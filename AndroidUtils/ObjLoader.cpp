...
namespace utils
{
		...
		while ( std::getline( file, line ) )
		{
			...
			else if ( select )
			{
				...
				else if ( ident == "map_Kd" )
				{
					std::string path;
					stream >> path;
					auto content = activity.getFileBinaryContent( path, true );
	...
	bool doPreloadFile( AndroidApp const & activity
		, std::string const & fileContent
		, render::MaterialList & materialsList
		, render::TextureList & texturesList
		, uint16_t & nv
		...
		, render::UInt16Array & faces )
	{
		...
		if ( !mtlfile.empty() )
		{
			loadMtlFile( activity
				, activity.getFileTextContent( mtlfile, true )
				, materialsList
				, texturesList );
		}
	...
	void doLoadNewMesh( AndroidApp const & activity
	...
	void doLoadMeshMaterials( AndroidApp const & activity
		, std::string const & fileContent
		, render::MaterialList & materialsList
		, render::TextureList & texturesList
		, render::MaterialArray & materials )
